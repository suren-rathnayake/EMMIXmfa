...
X <- X[,1]
expect_that(mctfa(X, g, q=1),
            throws_error("The number of factors must be less than the number of variables."))

X <- Y
expect_that(mctfa(X, g, ncol(X)),
            throws_error("The number of factors must be less than the number of variables."))


expect_that(mctfa(X, g, ncol(X)+1),
            throws_error("The number of factors must be less than the number of variables."))

expect_that(mctfa(X, g = g, q = -1),
            throws_error("q must be a positive integer."))


expect_that(mctfa(X, g = -2, q = q),
            throws_error("g must be a positive integer."))


expect_that(mctfa(X, g = 2.5, q = q),
            throws_error("g must be a positive integer."))


expect_that(mctfa(X, g = g, q = 1.5),
            throws_error("q must be a positive integer."))

test_that("mctfa with itmax NULL", {
  expect_that(mctfa (Y, g, q, itmax=NULL), throws_error())
})
...
test_that("mctfa with itmax neg", {
  expect_that(mctfa (Y, g, q, itmax=-1), throws_error())
})
